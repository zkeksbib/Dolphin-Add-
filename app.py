...
def mask_number(number):
    clean_num = str(number).replace("+", "").strip()
    if len(clean_num) > 6:
        return "+" + clean_num[:4] + "****" + clean_num[-2:]
    return "+" + clean_num
...
def is_within_last_2_days(date_str):
    """Filters out any entry older than 48 hours."""
    if not date_str:
        return True  # Process if date is missing/unknown
    ...
    for fmt in date_formats:
        try:
            entry_time = datetime.strptime(str(date_str).strip()[:19], fmt)
            if datetime.now() - entry_time <= timedelta(days=2):
                return True
            else:
                return False  # Older than 2 days
        except ValueError:
            continue
...
def format_message(phone, otp, message, service_name="WhatsApp", custom_time=None, custom_country=None, custom_masked=None):
    clean_msg = str(message).replace("\\n", "\n").replace("nn", "\n")
    masked = custom_masked if custom_masked else mask_number(phone)
    country = custom_country if custom_country else get_country(phone)
    pkt_time = custom_time if custom_time else datetime.now().strftime("%Y-%m-%d %H:%M:%S")
...
# ------------------ PROCESS ENTRIES ------------------
def process_panel_entries(entries, panel_name):
    for entry in entries:
        ...
        # FILTER: Skip entries older than 2 days
        ...
        if send_telegram(final_msg):
            ...
            print(f"\033[92mSUCCESS ({panel_name})\033[0m : \033[96m{otp}\033[0m")
...
# ====== AUTOMATIC SPECIFIC TEST SMS ON LAUNCH ======
print("🛠 SENDING SPECIFIC TEST SMS TO TELEGRAM GROUP...")
test_raw_msg = "Telegram code 27292\n\nYou can also tap on this link to log in\nhttpstmelogin27292\n\nO2P2zjBpJ"
test_formatted = format_message(
    phone="213700000027", 
    otp="27292", 
    message=test_raw_msg, 
    service_name="WhatsApp",
    custom_time="2026-07-27 13:21:37",
    custom_country="🇩🇿 Algeria",
    custom_masked="+2137****27"
)
...
# ====== INITIAL FETCH: LAST 2 DAYS MESSAGES ONLY ======
print("📥 FETCHING PAST 2 DAYS DATA FROM ALL PANELS...")
for panel in PANELS:
    entries = fetch_sms(panel)
    process_panel_entries(entries, panel.get("name", "Standard Panel"))

print("✅ INITIAL DATA SYNC COMPLETE. CONTINUOUS LIVE MONITORING STARTED...")
