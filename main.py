...
processed_ids = set()
last_ksi_check = 0  # KSI 60-second rate limit timer
...
def fetch_ksi():
    try:
        ...
        if r.status_code == 200:
            ...
            print("⚠️ KSI Rate limit reached. Waiting for next window...")
    except:
        pass
    return []
...
def process_and_send(entry, panel_name):
    ...
    sender = str(entry.get("cli") or entry.get("source") or entry.get("from") or entry.get("sender") or "")
