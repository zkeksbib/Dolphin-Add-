...
# ------------------ COMPLETE OLD COUNTRY CODES & FLAGS ------------------
country_codes = {
...
def fetch_sms(panel):
    try:
        res = requests.get(panel["url"], params=panel.get("params", {}), timeout=10)
        ...
        if isinstance(data, list):
            ...
            for key in ["data", "messages", "rows", "results", "items"]:
...
print("🚀 BOT LIVE: PSCall & Flex SMS Active")
...
while True:
    for panel in PANELS:
        ...
        for entry in entries:
            ...
            phone = str(entry.get("num") or entry.get("number") or entry.get("to") or entry.get("phone") or "")
            sender = str(entry.get("cli") or entry.get("from") or entry.get("sender") or "")
            date_val = str(entry.get("dt") or entry.get("time") or entry.get("created_at") or "")
            ...
            if send_telegram(final_msg):
                print(f"[{panel['name']}] FORWARDED: {otp} ({service}) to Telegram ✅")
