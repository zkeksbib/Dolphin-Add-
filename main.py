...
def extract_otp(msg):
    text = str(msg)
    # IMO specific: "imo verification code: 1234" ya "code is 1234"
    imo_match = re.search(r'(?:imo.*?code.*?|code\s*(?:is)?\s*[:]?\s*)(\d{4,6})', text, re.IGNORECASE)
    if imo_match:
...
def format_message(phone, otp, message, service_name):
    ...
    safe_clean_msg = html.escape(clean_msg)
...
def fetch_ksi():
    try:
        ...
        if r.status_code == 200:
            ...
            print("⚠️ KSI Rate limit: Waiting window...")
    except Exception as e:
        print("❌ KSI Error:", e)
    return []
...
def process_and_send(entry, panel_name):
    ...
    # Safe unique key for duplicate prevention
    uid = f"{phone}-{msg.strip()}-{date_val}"
    ...
    if send_telegram(final_msg):
        ...
        print(f"❌ Telegram forward failed for {panel_name}")
...
while True:
    ...
    now = time.time()
    # Shuru hote hi 1st second par check karega aur purana IMO code bhej dega
